...
void loop() {
    if(digitalRead(buttonStatus) == HIGH) {    //If the Button is pressed
    ...
    if(buttonStatus == 1) {    //If the value of the button variable is 1
        for(int i=0; i <= eeprom_record; i++) {    //Use a for loop to perform the save operation and increment the variable i from zero to 1000 (eeprom_register value).
            ...
            if(digitalRead(buttonPin) == HIGH) {    //If the Button is pressed during the recording process
                buttonStatus = 2;    //make the buttonStatus variable 2, end recording and proceed to repeat the play (playback)
                record_duration = i;    //When the button is pressed, transfer the value of the variable i to the record_duration variable
        ...
        for(int i = 0; i <= record_duration; i++) {    //Use a for loop to repeat the process and increment the variable i by one from zero to the record duration (record_duration).
