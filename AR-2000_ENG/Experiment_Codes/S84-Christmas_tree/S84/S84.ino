...
void loop() {
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 0; j < 5; j++) {
            ...
            digitalWrite(LEDs[randomNumber], LOW);    // Fixed: Changed HIGH to LOW for turning off LEDs
            digitalWrite(LEDs[(randomNumber * 2) % 5], LOW);    // Fixed: Changed HIGH to LOW
            digitalWrite(LEDs[(randomNumber * 5) % 6], LOW);    // Fixed: Changed HIGH to LOW
            digitalWrite(LEDs[(randomNumber * 8) % 7], LOW);    // Fixed: Changed HIGH to LOW
            delay(tempo / 20);    //Wait until tempo/20
        }
    }
}
