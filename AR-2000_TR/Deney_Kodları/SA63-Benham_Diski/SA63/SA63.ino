...
// LM293'un INPUT4 bacağının bağlandığı pin numarasını tanımla. Arduino 10. pinden LM293D 
// entegresinin 15. bacağına bağlı pin yardımı ile motor saat yönünün tersinde dönecektir 
int MotorPwm2 = 10;    

// LM293'un INPUT1 bacağının bağlandığı pin numarasını tanımla. Arduino 9. pinden LM293D 
// entegresinin 2. bacağına bağlı pin yardımı ile motor saat yönünde dönecektir  
int MotorPwm1 = 9;     
...
void loop() {
    ...
    if (butondurumu == 1) {
        analogWrite(MotorPwm1, 0);        // 9 numaralı pini toprağa çek (saat yönünün tersine dönüş için)
        analogWrite(MotorPwm2, oranla);   // 10 numaralı pine enerji ver
    }
    else if (butondurumu == 2) {       
        analogWrite(MotorPwm1, oranla);   // 9 numaralı pine enerji ver (saat yönünde dönüş için)
