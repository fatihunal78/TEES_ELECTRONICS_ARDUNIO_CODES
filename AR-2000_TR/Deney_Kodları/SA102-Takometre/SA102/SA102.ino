...
const byte Pervane_kanatlari = 3;                  // Panel üzerindeki pervanede kaç kanat, kaç bıçak olduğunu belirten değeri tanımla
                                                  // Bizim pervanemiz, 3 bıcaklı olduğu için her bir dönüşte 3 pulse okur
...
volatile unsigned long Pulse_Arasi_Periyot = ZeroDebouncing + 1000;   // Her bir okunan pulse arasındaki periyodu hesapla
volatile unsigned long Ortalama_Periyot = ZeroDebouncing + 1000;      // PULSE'ler arası ortalama periyotun mikro saniye cinsinden 
                                                                      // tutulacağı değişkeni tanımla
...
unsigned int PulseSayac = 1;                      // Okunan pulse'larin tutulacağı değişken, tanımla, ilk değeri 1'e eşit
...
unsigned long Okuma[okuma_sayisi];                // Okunan değerlerin array olarak tutulması için değişkeni tanımla
unsigned long Okuma_indeks;                       // Kaç okuma yapıldığını anlamak için değişkeni tanımla
unsigned long toplam;                             // Toplam okumların tutulacağı değişkeni tanımla
...
int potPin = A0;                                 // Motoru kontrol edecek potansiyometrenin bağlandığı pini tanımla
...
void setup() 
{
    ...
    attachInterrupt(digitalPinToInterrupt(2), Kesme_Fonksiyonu, RISING);  // 2 nolu dijital pinden kesme okumak için
                                                                          // Kesme_Fonksiyonu içindeki olayları yükselen 
                                                                          // kenar(RISING) olduğu zaman gerçekleştir
    ...
    lcd.print("   TAKOMETRE  ");                // TAKOMETRE yaz
...
void loop()  
{   
    // Motorun maksimum hızda dönmemesi için PWM değeri 180 ile sınırlandırıldı
    potDegeri = map(analogRead(potPin), 0, 1023, 0, 180);  // Analog pinden okunan değerleri 0 ile 180 arasında oranla 
    analogWrite(motorPin, potDegeri);                       // Oranlanmış olan 0 ile 180 arasındaki değerleri motora gönder 
    ...
    Frekans_Ondalik = 10000000000 / Ortalama_Periyot;  // Pulse'lar arasındaki zamanı ondalık olarak Frekans_Ondalik 
                                                        // değişkenine kaydet 

    // Eğer motor dönmeyi durdurduysa veya çok yavaş dönüyorsa, okunan frekans değerini 0 olaarak kaydet
    ...
    Frekans_tamsayi = Frekans_Ondalik / 10000; // Hesaplanan Frekans_Ondalik değişkenini tam sayıya çevir ve Frekans_tamsayi 
                                               // değişkenine kaydet

    // RPM (Dakikadaki Devir)'i hesapla
    RPM = Frekans_Ondalik / Pervane_kanatlari * 60;  // Deviri hesaplamak için Frekans_Ondalik değişkenindeki değeri 
                                                     // Pervane_kanatlari ile böl çünkü 3 pulse okumasını istemiyor.
                                                     // Pervane_kanatlari değişkenine bölmezsek dakikadaki devir sayısı 3 katı çıkar
                                                     // 1 dakikadaki değerini bulmak için 60 ile çarp (1 dakika = 60 saniye)
                                                 
    RPM = RPM / 10000;                         // Tam sayıya çevirmek ve mikro saniye cinsinden olan RPM değişkenini saniye'ye 
                                              // cevirmek için 10000'e böl 
    ...
    if (yenizaman - eskizaman > 2000)
    {
        ...
        lcd.print(" Devir / Dakika");          // Dakikadaki Devir yaz
        lcd.setCursor(4, 1);                   // Alt satıra geç 
        lcd.print(ortalama);                   // Ortalama değişkeninde tutulan Ortalama RPM değerini yaz
        lcd.print(" RPM");                     // RPM yaz
        eskizaman = yenizaman;
    }
    delay(200);
}  

void Kesme_Fonksiyonu()                        // Pulseları okuyabilmek için kesme fonksiyonu tanımla
{
    ...
    {
        Ortalama_Periyot = Toplam_Periyot / Okuma_Miktari;  // Ortalama periyotu bulmak için Toplam_Periyot değişkenini 
                                                            // Okuma_Miktari değişkenine böl
        
        PulseSayac = 1;                       // Okunan pulse'ları sayan PulseSayac değişkenini 1'e eşitle 
        Toplam_Periyot = Pulse_Arasi_Periyot; // Yeni ortalama alabilmek için Toplam_Periyot değişkenini Pulse_Arasi_Periyot 
                                             // değişkenine eşitle

        int oranla = map(Pulse_Arasi_Periyot, 40000, 5000, 1, 10);  // Pulse_Arasi_Periyot değişkeninden okunan 40000 ile 
                                                                     // 5000 arası değeri 1 ile 10 arasında oranla
    ...
    else
    {
        ...
        Toplam_Periyot = Toplam_Periyot + Pulse_Arasi_Periyot;  // Toplam_Periyot değişkenini önceki değeri ile 
                                                                // Pulse_Arasi_Periyot topla
    }
}  
